...
Demo Scenarios
Customer makes a purchase → Added to "Active Buyers" → UI updates instantly → Campaign simulator logs the notification

Customer goes inactive for 30+ days → Removed from "Active Buyers" → Churn event logged

VIP segment changes → "VIP At Risk" segment auto-re-evaluates → Both publish independent deltas

Bulk import of 500 customers → Batched into ~5 events → Queue stays healthy → UI updates smoothly

Static segment stays frozen → "March Campaign" ignores all data changes → Only updates on manual refresh
...
curl -X POST http://localhost:3000/segments \
  ...
  -d '{
    "name": "Active Buyers",
    "type": "DYNAMIC",
    "rules": [{"field": "lastTransactionDays", "operator": "<=", "value": 30}]
  }'
